...
解决此问题的方法就是必须修改服务器中所有含有玩家UUID的文件，将在线UUID改为离线后的UUID。
...
1. `Info.json`
   ...
   *必须，这是你要配置的文件，需要你自己增添每位服务器玩家的UUID，配置需要改UUID文件名的地方，以及是否需要修改那些文件内的UUID内容
...
4. 打开`Info.json`，根据你的服务器玩家UUID和需要修改的文件进行配置
    ...
    - `changeUUID_folder_name`是你需要修改UUID的文件夹名称，需要根据实际情况和是否需要修改文件中的内容进行修改。
       例如：原版存档中只需要修改`advancements`、`playerdata`、`stats`这三个文件夹内每个文件名中的UUID部分，模组存档例如安装了ftb相关的既要修改文件中的UUID，也需要打开这些文件修改替换文件内的UUID

    - `player`是玩家信息，这里你需要填写玩家的昵称（name），`Online-Mode=true`时的UUID（`Online_uuid`），以及`Online-Mode=false`时的UUID（`Offline_uuid`）。
